...
##########################################################################################################
		while read thdline
		do
			ls $data_path/$arg/Wavform/${thdline}* > $current_path/tmp/$arg/_${thdline}.tfile
			while read _thdline
			do
				echo "`basename $_thdline`" >> $current_path/tmp/$arg/${thdline}.tfile
			done < $current_path/tmp/$arg/_${thdline}.tfile
			rm -rf $current_path/tmp/$arg/_${thdline}.tfile

			while read testfileline
			do
				_testfileline=`basename $testfileline .wav`
				awk '$1 ~ /'$_testfileline'\.wav/ {print $2 >> "'$current_path/tmp/$arg/${arg}.tdata'"}' $current_path/tmp/$arg/${arg}.tmp
			done < $current_path/tmp/$arg/${thdline}.tfile
		done < $data_head_path/$arg/$arg.thd
