...
if [ $# != 6 ]; then
   echo "Usage: $0 <mlp_dir> <data_scp> <data_cv_scp> <class_scp> <class_cv_scp> <class_num>"
...
labels_tr=scp:${alidir}
labels_cv=scp:${alidir_cv}

mkdir -p $dir/{log,nnet}

cat $data_scp > $dir/train.scp
...
if [ ! -z "$mlp_init" ]; then
  ...
  mv $dir/nnet.init $dir/nnet.init_1
  cat $dir/nnet.init_1 | sed "s/<softmax>/<sigmoid>/g" > $dir/nnet.init
  rm $dir/nnet.init_1
	
  #optionally prepend dbn to the initialization
  if [ ! -z $dbn ]; then
    mlp_init_old=$mlp_init; mlp_init=$dir/nnet_$(basename $dbn)_dnn.init
    nnet-concat --binary=false $dbn $mlp_init_old $mlp_init 
  fi
fi



bash ${tools}/train_nnet_scheduler_mat.sh \
