...
max_rejections=5
min_iters=
start_halving_inc=0.01
end_halving_inc=0.001
halving_factor=0.5
# misc.
verbose=1
# gpu
...
$train_tool --cross-validate=true \
 --bunchsize=$bunch_size --cachesize=$cache_size --verbose=$verbose --binary=false \
...
acc=$(cat $dir/log/prerun.log | awk 'BEGIN{FS=":"} /err\/frm:/{ xent = $NF; } END{print xent}')
echo "avg.mse$(printf "%.4f" $acc), "
...
for iter in $(seq -w $max_iters); do
  ...
  $train_tool \
   ...
   --bunchsize=$bunch_size --cachesize=$cache_size --randomize=true --verbose=$verbose --binary=false \
   ${feature_transform:+ --feature-transform=$feature_transform} \
   ${use_gpu_id:+ --use-gpu-id=$use_gpu_id} \
   ${seed:+ --seed=$seed} \
  ...
  tr_acc=$(cat $dir/log/iter$iter.log | awk 'BEGIN{FS=":"} /err\/frm:/{ xent = $NF; } END{print xent}')
  echo -n "avg.mse$(printf "%.4f" $tr_acc),lrate$(printf "%.6g" $learn_rate), "
  ...
  acc_new=$(cat $dir/log/iter$iter.log | awk 'BEGIN{FS=":"} /err\/frm:/{ xent = $NF; } END{print xent}')
  echo -n "avg.mse$(printf "%.4f" $acc_new), "
  ...
  #stopping criterion
  if [[ ("0" == "$rejecting" || "$max_rejections" < "$rejecting") && "1" == "$halving" && "1" == "$(awk "BEGIN{print($acc > $acc_prev-$end_halving_inc)}")" ]]; then
    if [[ "$min_iters" != "" ]]; then
